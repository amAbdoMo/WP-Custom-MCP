...
class McpCustomPostTypesTools {
	 ...
	 * @return bool
	 */
	public function search_custom_post_types_permission_callback( array $params = array() ): bool {
		$post_type = isset( $params['post_type'] ) ? sanitize_key( $params['post_type'] ) : '';

		return $this->current_user_can_for_post_type( $post_type, 'edit_posts' );
	}
	...
	public function get_custom_post_type( array $params ): array {
		...
		if ( ! $post || $post->post_type !== sanitize_key( $params['post_type'] ) ) {
			return array(
				'error' => array(
					'code'    => -32000,
	 ...
	 * @return bool
	 */
	public function get_custom_post_type_permission_callback( array $params = array() ): bool {
	 ...
	 * @return bool
	 */
	public function add_custom_post_type_permission_callback( array $params = array() ): bool {
		$post_type = isset( $params['post_type'] ) ? sanitize_key( $params['post_type'] ) : '';

		return $this->current_user_can_for_post_type( $post_type, 'create_posts' );
	}
	 ...
	 * @return bool
	 */
	public function update_custom_post_type_permission_callback( array $params = array() ): bool {
		$post_id = isset( $params['id'] ) ? absint( $params['id'] ) : 0;
	 ...
	 * @return bool
	 */
	public function delete_custom_post_type_permission_callback( array $params = array() ): bool {
		$post_id = isset( $params['id'] ) ? absint( $params['id'] ) : 0;
	...
	/**
	 * Check a post type capability for the current user.
	 *
	 * @param string $post_type Post type name.
	 * @param string $capability_property Capability property on the post type object.
	 * @return bool
	 */
	private function current_user_can_for_post_type( string $post_type, string $capability_property ): bool {
		if ( ! post_type_exists( $post_type ) ) {
			return false;
		}

		$post_type_object = get_post_type_object( $post_type );
		if ( ! $post_type_object || empty( $post_type_object->cap->{$capability_property} ) ) {
			return false;
		}

		return current_user_can( $post_type_object->cap->{$capability_property} );
	}
}
