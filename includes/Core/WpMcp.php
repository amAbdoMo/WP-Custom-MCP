...
use Automattic\WordpressMcp\Tools\McpFileDevelopmentTools;
use Automattic\WordpressMcp\Tools\McpSiteInfo;
use Automattic\WordpressMcp\Tools\McpWoodmartWpmlTools;
use Automattic\WordpressMcp\Tools\McpWpmlCheckoutTranslationTools;
use Automattic\WordpressMcp\Tools\McpWpmlElementorManifestTools;
use Automattic\WordpressMcp\Tools\McpWpmlElementorTranslationTools;
use Automattic\WordpressMcp\Tools\McpWpmlFrontendVerificationTools;
use Automattic\WordpressMcp\Tools\McpWpmlRelationshipTools;
use Automattic\WordpressMcp\Tools\McpWpmlSiteProfileTools;
use Automattic\WordpressMcp\Tools\McpWpmlStringTranslationTools;
use Automattic\WordpressMcp\Tools\McpWpmlTranslationKnowHow;
use Automattic\WordpressMcp\Tools\McpWpmlWooCommerceAuditTools;
...
use Automattic\WordpressMcp\Prompts\McpWpmlTranslationWorkflow;
...
class WpMcp {
	 ...
	 * @var array
	 */
	private array $all_resources = array();
	 ...
	 * @var array
	 */
	private array $all_prompts = array();
	 ...
	 * @var string
	 */
	private const RESOURCE_STATES_OPTION = 'wordpress_mcp_resource_states';
	 ...
	 * @var string
	 */
	private const PROMPT_STATES_OPTION = 'wordpress_mcp_prompt_states';
	...
	private function init_default_tools(): void {
		...
		new McpRestApiCrud();
		new McpWpmlSiteProfileTools();
		new McpWpmlRelationshipTools();
		new McpWpmlStringTranslationTools();
		new McpWpmlTranslationKnowHow();
		new McpWpmlElementorManifestTools();
		new McpWpmlElementorTranslationTools();
		new McpWpmlCheckoutTranslationTools();
		new McpWpmlWooCommerceAuditTools();
		new McpWpmlFrontendVerificationTools();
		new McpWoodmartWpmlTools();
		new McpFileDevelopmentTools();
	}
	...
	private function init_default_prompts(): void {
		...
		new McpAnalyzeSales();
		new McpWpmlTranslationWorkflow();
	}
	...
	public function register_tool( array $args ): void {
		...
		$args['disabled_by_rest_crud'] = false;
	...
	public function register_resource( array $args ): void {
		$args['resource_enabled'] = $this->is_resource_enabled( $args['uri'] );
		$this->all_resources[ $args['uri'] ] = $args;

		if ( ! $args['resource_enabled'] ) {
			return;
		}
	...
	public function register_resource_callback( string $uri, callable $callback ): void {
		if ( ! $this->is_resource_enabled( $uri ) ) {
			return;
		}
	...
	public function register_prompt( array $prompt, array $messages ): void {
		$name = $prompt['name'];
		$prompt['prompt_enabled'] = $this->is_prompt_enabled( $name );
		$this->all_prompts[ $name ] = $prompt;

		if ( ! $prompt['prompt_enabled'] ) {
			return;
		}
	...
	/**
	 * Get all resources with enabled state.
	 *
	 * @return array
	 */
	public function get_all_resources(): array {
		$resource_states = get_option( self::RESOURCE_STATES_OPTION, array() );
		$resources       = $this->all_resources;

		foreach ( $resources as &$resource ) {
			$uri = $resource['uri'] ?? '';
			$resource['enabled'] = ! isset( $resource_states[ $uri ] ) || $resource_states[ $uri ];
		}
	...
	/**
	 * Get all prompts with enabled state.
	 *
	 * @return array
	 */
	public function get_all_prompts(): array {
		$prompt_states = get_option( self::PROMPT_STATES_OPTION, array() );
		$prompts       = $this->all_prompts;

		foreach ( $prompts as &$prompt ) {
			$name = $prompt['name'] ?? '';
			$prompt['enabled'] = ! isset( $prompt_states[ $name ] ) || $prompt_states[ $name ];
		}
	 ...
	 *
	 * @param string $resource_uri The resource URI to check.
	 * @return bool Whether the resource is enabled.
	 */
	public function is_resource_enabled( string $resource_uri ): bool {
		$resource_states = get_option( self::RESOURCE_STATES_OPTION, array() );
		return ! isset( $resource_states[ $resource_uri ] ) || $resource_states[ $resource_uri ];
	}

	/**
	 * Check if a prompt is enabled.
	 *
	 * @param string $prompt_name The prompt name to check.
	 * @return bool Whether the prompt is enabled.
	 */
	public function is_prompt_enabled( string $prompt_name ): bool {
		$prompt_states = get_option( self::PROMPT_STATES_OPTION, array() );
		return ! isset( $prompt_states[ $prompt_name ] ) || $prompt_states[ $prompt_name ];
	}
}
