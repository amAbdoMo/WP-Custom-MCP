...
class JwtAuthTest extends WP_UnitTestCase {
	...
	/**
	 * Test token generation denies low-privilege users.
	 */
	public function test_generate_token_denies_subscriber_credentials(): void {
		$request = new WP_REST_Request( 'POST', '/jwt-auth/v1/token' );
		$request->set_body(
			wp_json_encode(
				array(
					'username' => $this->subscriber_user->user_login,
					'password' => 'test_password',
				)
			)
		);
